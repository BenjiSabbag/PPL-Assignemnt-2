...
;;         |  ( cond <cond-clauses>*> )     / CondExp(cond-clausse:List(cond-clause)))
...
export const parseCondExp = (params: Sexp[]): Result<CondExp> => {
    if (!allT(isArray, params)) {
        return makeFailure("Invalid cond clauses");
    } else {
        if (params.length < 2){
            return makeFailure("Not enough cond clauses");
        }
        if (params[params.length-1][0] != "else"){
            return makeFailure("Missing valid else clause");
        }
        const clausesResult = mapResult((clause: Sexp[]) =>
            safe2((test: CExp, then: CExp[]) => makeOk(makeCondClause(test, then)))
                    (parseL31CExp(first(clause)), mapResult(parseL31CExp, rest(clause))), params);
...
const unparseCondExp = (ce: CondExp) : string => 
    `(cond ${map(unparseCondClause, ce.condclauses).join(" ")})`

const unparseCondClause = (cc: CondClause) : string =>
    `(${unparseL31(cc.test)} ${unparseL31(cc.then[0])})`
